...
keywords = ['crystal', 'basis', 'strain',  'cpg', 'normal', 'directory', 'kineticrange', 'uniquepos', 'species',
            'bulkspecies', 'jumpmech', 'iniconf', 'printxyz', 'printsymeqs', 'runnum', 'kiraloop', 'ballistic',
            'writesymbolic', 'compact', 'limitspace']
...
if myinput[-4:] != ".txt":
    copy_input_path += ".txt"
if input_dataset['directory'][1] == "cwd" or input_dataset['directory'][1] == "./":
...
# Print unknown keywords if any was found in the input file.
for k in unknownkeys:
    logger.info("!! Unknown keyword {} in the input file.".format(k))
del unknownkeys
...
if input_dataset['writesymbolic'] is not None:
    logger.info("!! WRITESYMBOLIC keyword is used: can be very time and memory consuming!")
    writesymbolic = True
else:
    writesymbolic = False
...
if input_dataset['compact'] is not None:
    if len(input_dataset['compact']) != 3:
        input_dataset['compact'] = [0, [-1, 1e6]]  # default values
        logger.info("!! COMPACT keyword must have three arguments! It is ignored for this calculation.")
    else:
        input_dataset['compact'] = [int(float(input_dataset['compact'][0])), [float(input_dataset['compact'][1]), float(input_dataset['compact'][2])]]
        if input_dataset['compact'][0] < 0:
            input_dataset['compact'][0] = 0
            logger.info("!! The first number in the COMPACT keyword must be positive.")
        input_dataset['compact'][1] = sorted(input_dataset['compact'][1])
        if input_dataset['compact'][1][1] < KiRa:
            logger.info("!! The second number in the COMPACT keyword should in principle be larger than the kinetic range.")
        logger.info("!! COMPACT keyword is used. Only {:.0f} bond(s) with distances lower than {:.2f} or larger than {:.2f} are allowed between the components.".format(input_dataset['compact'][0], *input_dataset['compact'][1]))
else:
    input_dataset['compact'] = [0, [-1,1e6]] #default values
...
for ba in range(n_basis_atoms):
    ...
    basis_list[ba][1:4] = kp.apply_pbc(basis_list[ba][1:4])
crystal.set_basislist(basis_list)
crystal.set_atomicvolume()

# Checking if diffusion is limited in space, and re-defining the crystal supercell if needed
if input_dataset['limitspace'] is not None: # first check the format of the keyword arguments
    if len(input_dataset['limitspace']) != 8:
        logger.info("!! Wrong number of inputs in the LIMITSPACE keyword. Check format.")
        input_dataset['limitspace'] = None
    else:
        if input_dataset['limitspace'][0] != 's' and input_dataset['limitspace'][0] != 'o':
            logger.info("!! First argument of LIMITSPACE keyword must be either s or o")
            input_dataset['limitspace'] = None
        if input_dataset['limitspace'][4] != 'e' and input_dataset['limitspace'][4] != 'r':
            logger.info("!! Fifth argument of LIMITSPACE keyword must be either r or e")
            input_dataset['limitspace'] = None
        if input_dataset['limitspace'][4] != 'e' and input_dataset['limitspace'][0] != 's':
            logger.info("!! Ellispoid shape in supercell coordinates is not implemented in LIMITSPACE keyword")
            input_dataset['limitspace'] = None
if input_dataset['limitspace'] is not None:
    center = np.array([kp.evalinput(a) for a in input_dataset['limitspace'][1:4]], dtype=float)
    limits = np.array([kp.evalinput(a) for a in input_dataset['limitspace'][5:8]], dtype=float) # deal with i
    if input_dataset['limitspace'][0] == 'o': # write everything in supercell coordinates
        center = kp.change_base(arr=center, crystal=crystal)
        limits = kp.change_base(arr=limits, crystal=crystal)
...
if 'd' in ''.join(input_dataset['uniquepos']): # dumbbell calculation
    ...
    for i in range(0, int(input_dataset['uniquepos'][0])):
        if 'd' in ''.join(input_dataset['uniquepos'][i*4+2:i*4+5]):  # this defect is a dumbbell
            ...
            for k in range(0, 3):
                dbcoords.append(kp.evalinput(input_dataset['uniquepos'][i*4+2+k])-kp.evalinputd0(input_dataset['uniquepos'][i*4+2+k]))
...
if strain_flag:
    ...
    input_dataset, brokenjumps = kp.produce_strain_input(dataset=input_dataset, input_file=myinput)
...
if input_dataset['printsymeqs'] is not None:
    pf="{:16.12f} {:16.12f} {:16.12f}"  # format with a fiven print precision (number of decimals)
    nobulk = "bulk"
...
if input_dataset['iniconf'] is not None:
    if not (int(input_dataset['iniconf'][0]) == n_components):
        ...
        iniconf_label, def_list, trans_list, _ = kp.set_confname(defects=def_list, translations=trans_list, perms=name_perms,species=[c.get_species for c in component_list])
        ...
        if not kp.check_configuration_consistence(label=iniconf_label, component_list=component_list, all_defect_list=all_defect_list, crystal=strcrystal):
            ...
            connex, _, _, myinikira = kp.check_connectivity(configuration=myiniconf, kira=KiRa, crystal=strcrystal,
                                            compactN=input_dataset['compact'][0], compactD=input_dataset['compact'][1])
            if not connex:
...
if (input_dataset['iniconf'] is None) or (not inivalid):
    ...
    if len(component_list) > 1:
        ...
        iterlist = [np.hstack((np.array(x, dtype=int),np.zeros(3-crystal.get_dim(), dtype=int))) for x in product(range(-3, 4), repeat=crystal.get_dim())]
...
myiniconf.findjumps(alljumps=jump_list, components=component_list, crystal=crystal, all_defect_list=all_defect_list,
                    KiRa=KiRa, name_perms=name_perms, compactN=input_dataset['compact'][0], compactD=input_dataset['compact'][1])
...
# while(thermoidx)<1000: #other possibility to stop the loop
    ...
    # making sure that the configuration is not dissociated
    elif tmp_list[0][0][0] == 'd': # new dissociated configuration
        ...
        newdisso.find_subconfigurations(component_list=component_list, crystal=crystal, ThRa=ThRa,
                                        compactN=input_dataset['compact'][0], compactD=input_dataset['compact'][1])  # looking for subconfigurations
        ...
        del tmp_list[0] # no exploration from dissociated configurations
    else:
        ...
        # For each symmetry unique interaction, look for all jumps and accessible configurations
        results[1][0][0].findjumps(alljumps=jump_list, components=component_list, crystal=crystal,
...
if n_components > 1:  # if monomer, than site interactions and effective interactions are similar; no need for both
    ...
    del sudefect
# siteinter_index = 0  # shortcut to remove all site interactions ?
    ...
    for conf in threp_list:
        ...
        if len(conf[0].get_subconfigurations()) == 0:  # we already know for sure that it is symmetrically unique
            ...
            name, subconfig_list, subperms = conf[0].findsubname(species_list=species_list,
                                                                  ...
                                                                  symop_list=symop_list, symop_cpg_list=symop_cpg_list,
                                                                  subconfig_list=subconfig_list, subperms=subperms)
            if disso_list.get(name) is None: #we must create this new dissociated configuration
                thermoidx += 1
                logger.info("{} {}".format(str(thermoidx), name))
                disso_list[name] = copy.copy(conf[0])
                disso_list[name].reset_disso()
                config_list[name] = disso_list[name]
                disso_list[name].set_thermoint(thermoidx)
                th2label[str(thermoidx)] = disso_list[name].get_label()
                thconfig_list[disso_list[name].get_label()] = disso_list[name]
...
if input_dataset['ballistic'] is not None:
    logger.info("!! BALISTIC keyword is used, a jump and its reverse jump are not connected by microscopic detailed balance")
    balcalc = True
else:
    balcalc = False
logger.info("Searching for jump frequencies (starts at {:.3f} s)".format(tm.time() - start_time))
Nfreq = 0
for conf in threp_list:  # for all sample configurations of kinetic interactions
    for idx, jump in enumerate(conf[0].get_jumplist()):
        freq_output = kp.id_jump_freq(config1=conf[0], config2=config_list[jump[0]], jumpmech=jump[1],
                                      freq_list=freq_list, symop_list=symop_list, balcalc=balcalc,
        ...
        if balcalc and jump[4].get_disso():  # add reverse dissociated jump freq
            freq_output = kp.id_asso_freq(config1=config_list[jump[0]], config2=conf[0], jumpmech=jump[1],
...
for freq0 in freq_list:
    for freq in freq_list[freq0]:
        ...
        if freq.get_disso():
            if freq.get_config_fin().get_subname() is not None:
                dissotype = '|'.join([a.split(sep='s')[0] for a in freq.get_config_fin().get_subname()[1:].split(sep='|')])
            else:
                dissotype = '|'.join([a.split(sep='s')[0] for a in freq.get_config_ini().get_subname()[1:].split(sep='|')])
...
# Apply symmetry operations to set jump list for configurations other than thermodynamic representants
for conf in kirep_list+null_kirep_list:
    if len(conf[0].get_jumplist()) == 0: #else we already have the jumps
        conf[0].jumplist_from_symmetry(thconf=threp_list[conf[0].get_posthreplist()][0], symops=symop_list, all_defect_list=all_defect_list, name_perms=name_perms)
...
if len(maxkira.keys())>0:
    ...
    logger.info("!! Will not be able to perform the convergence study (No radius values found in between the thermal and the kinetic range or KIRALOOP keyword not found)")
...
for interaction in kirep_list:
    ...
    for conf_fin in interaction[0].get_jumplist():  # Loop over all final configurations that are reached from initial configuration "interaction"
        # T matrix
        #  conf_fin = [ 0:final configuration label; 1:jump mechanism; 2:kira for this configuration;
        #  3:delta translation vectors; 4:jump frequency; 5:list of defects in final configuration ]
        Tvector[0, eq_idx] += - conf_fin[4].get_label()*eq_sign  # Contribution from initial configuration (negative) (conf_fin[4] contains the Jump Frequency W[x,x]
    ...
    if np.all(Kvec[:, 0:n_species] == 0) and np.all(Tvector[0, 1:eq_idx]==0) and np.all(Tvector[0, eq_idx + 1:]==0):
        ...
        if balcalc:
            for b, val in enumerate(Tvector[0][:eq_idx]):
                if val != 0:
                    Tsparse.append(val*interaction[1])
                    Tcoords.append((eq_idx-1-len(removedint), b-1))
    ...
    for freq0 in freq_list:
        for freq in freq_list[freq0]:
            ...
            if freq.get_disso():
                ...
                tofile += ["     "]
            output.writelines("{:s}  {:s}  {:s} {:s} {:s} {:s} {:s} {:s}\n".format(*tofile))
...
# Creating passage matrix to compute steady-state
if balcalc:
    logger.info("Constructing transition matrix (starts at {:.3f} s)".format(tm.time() - start_time))
    transmat = np.zeros((len(thconfig_list), len(thconfig_list)), dtype=object)
    for conf in threp_list:
        for jump in conf[0].get_jumplist():
            i_dep = jump[4].get_config_ini().get_thermoint()
            i_arr = jump[4].get_config_fin().get_thermoint()
            if i_dep!=i_arr:
                transmat[i_dep][i_arr] += len(conf[1])*jump[4].get_label()
                transmat[i_arr][i_arr] -= len(conf[1])*jump[4].get_label()
                if jump[4].get_disso(): #add reverse dissociated jump freq
                    name = 'd'+str(i_arr)+'_'+str(i_dep)+'_'+str(jump[1].get_index())
                    transmat[i_arr][i_dep] += len(conf[1])*freq_list[name][0].get_label()
                    transmat[i_dep][i_dep] -= len(conf[1])*freq_list[name][0].get_label()
else:
    transmat = []
...
# Create a dictionnary with the net displacements for all symmetry equivalents of jump mechanisms
jumpmechdisp = {}
# sp = list(set([e.get_species().get_index() for e in component_list]))
for e in jump_list:
    if jumpmechdisp.get(e.get_name()) is None:
        jumpmechdisp[e.get_name()] = [np.array(e.get_netdisp()[1:], dtype=float)]
        # jumpmechdisp[e.get_name()] = [np.array([e.get_netdisp()[i] for i in sp], dtype=float)]
    else:
        jumpmechdisp[e.get_name()].append(np.array(e.get_netdisp()[1:], dtype=float))
        # jumpmechdisp[e.get_name()].append(np.array([e.get_netdisp()[i] for i in sp], dtype=float))
...
pickle.dump([Tcoords, Tsparse, Tdiag, L0matrix, Klambda, z_function,
         ...
         all_defect_list, species_list[-1], uniqueconf, uniquefreq,
             ...
             kp.maxdbdist*kp.db_displ, SiteCorr, disfmt, fnfmt, transmat, brokenjumps, jumpmechdisp], open(directory + 'analytical_kineclue_output.pkl', 'wb'), -1)

if writesymbolic:
    logger.info("Writting analytical expressions...(starts at {:.3f} s)".format(tm.time() - start_time))
    kp.print_analytical_expressions(Td=Tdiag,Tc=Tcoords,Ts=Tsparse,Lo=L0matrix,Kl=Klambda, W=W,
                                    file=directory +"ANALYTICAL_EXPRESSIONS.txt", ndirs=Ndirs, bal=balcalc,
                                    replace=[a for a in input_dataset["writesymbolic"] if a[0]=="("],
                                    toprint=[a for a in input_dataset["writesymbolic"] if a[0]!="("],
                                    reversejumps=[])
